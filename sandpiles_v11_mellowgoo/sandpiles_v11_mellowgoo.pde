...
void setup() {
  ...
  colorMode(RGB);
  frameRate(100);
  sandpiles = new int[width][height]; //A 2-dimensional array
  for (int x = 0; x < width; x++) {
    for (int y = 0; y < height; y++) {
      sandpiles[x][y] = ceil( noise(x,y)*2 );
    }
  }
  sandpiles[width/2][height/2] = 100000000; //Initial sandpile will topple outward
