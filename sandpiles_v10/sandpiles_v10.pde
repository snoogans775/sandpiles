...
void draw() {
  ...
  for (int i = 0; i < 3; i++) {
   ...
   //However, using dynamic values based on time creates some extraordinary effects
   //Try using round(), sin(), or or other trig functions
   topple( floor(sin( tan( millis() ) - cos(frameCount*200) ) ) );
  }
}